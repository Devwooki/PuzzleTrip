<template>
    <div class="detail-div">
        <div class="detail-container">
                    ...
                    <div class="main-content">
                        ...
                        <div class="show-only-img" v-if="OnlyImage.length !== 0">
                            <div class="img-container" v-for="(img, idx) in OnlyImage" :key="idx">
                                <!--
                                안되는 이유가 .env파일에는 직접 사용(매핑)해서 쓸 수 없다-> 중요사항이 노출되니까
                                vue cli 개발 환경에서 .env파일이 아니면 녹스질을 한다.
                                <img :src="`${process.env.VUE_APP_API_BASE_URL}/profilePath/${img.saveFolder}/${img.saveFile}`">
                                -->
            ...
            <div class="btn-loc">
                <router-link class="write-btn btn-modi"
                             v-if="checkUserInfo !==null && (board.userId === checkUserInfo.id || checkUserInfo.id === 'admin') "
                             :to="{name : 'boardModify'}">수정
                </router-link>
                <a class="write-btn btn-del"
                   v-if="checkUserInfo !==null && (board.userId === checkUserInfo.id || checkUserInfo.id === 'admin')"
                   @click="beforeRemove">삭제</a>
